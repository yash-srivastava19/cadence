...
class TestPatching:
    def test_a_replacement_lands(self):
        patch = a_patch(" def solve(items):", "-    return 0", "+    return sum(items)")
    ...
    def test_several_hunks_all_land(self):
        ...
        assert apply_patch(BASELINE, patch) == "def solve(xs):\n    return 1"

    def test_a_patch_that_matches_nothing_says_which_line(self):
        patch = a_patch(" def solve(items):", "-    return 99", "+    return 1")
...
class TestFailuresAreDistinguished:
    def test_a_crash_is_a_crash(self):
        verdict = a_runner().try_(
            Candidate(code="def solve(items): raise ValueError('x')")
        )
